...
async def get_topics():
    ...
    try:
        topics = await asyncio.to_thread(topic_agent.get_top_topics, limit=12)
        frontend_topics = [
            {
                "id": topic.get('id'),
...
async def generate_article_ws(websocket: WebSocket):
    ...
    try:
        ...
        gap_report = await asyncio.to_thread(gap_agent.analyze_topic, selected_topic)
        if "error" in gap_report:
            raise Exception(f"Gap analysis failed: {gap_report['error']}")
        await asyncio.sleep(1)

        factual_briefing = await asyncio.to_thread(gap_agent.get_factual_briefing, topic_title)
        await asyncio.sleep(1)

        # Step 2: Outline Generation
        await websocket.send_json({"text": "Building strategic content outline...", "progress": 40})
        blog_outline = await asyncio.to_thread(outline_agent.create_outline, topic_title, gap_report, factual_briefing)
        if not blog_outline:
        ...
        first_draft = await asyncio.to_thread(writing_agent.write_article, blog_outline, factual_briefing)  # ← FIX: pass factual_briefing
        if not first_draft:
        ...
        seo_report = await asyncio.to_thread(seo_agent.inspector, first_draft, keywords)
        final_article = await asyncio.to_thread(seo_agent.rewrite_article, first_draft, seo_report)
